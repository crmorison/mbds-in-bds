...
def av_mbd_plot(rls, mbss, div, dd_for_pois):
    ...
    if (exists('wijs/Wij' + str(POP_F - POP_I) + '.pkl') == True):
        wijs = load_pickle('wijs/Wij' + str(int(POP_F - POP_I)) + '.pkl')
        pred_mbd_x = range(int(POP_F - POP_I))
        pred_mbd_y = [0] + [wijs['0,' + str(k)] * POP_F for k in range(1, int(POP_F - POP_I))]
        tail = 7
        a = np.maximum(0, np.nonzero([int(i) for i in pred_mbd_y])[0][0] - tail)
        b =  np.maximum(-1, np.nonzero([int(i) for i in pred_mbd_y])[0][-1] + tail)
    if div == 1:
    ...
    plt.plot(pred_mbd_x[a:b], pred_mbd_y[a:b], 'b', label = 'predicted')
    plt.legend(loc = 'upper right')
    tikzplotlib.clean_figure()
    plt.show()
    tikzplotlib_fix_ncols(fig)
    tikzplotlib.save('av_dd_plot2.tex')
    plt.close()
    return mb_avs
